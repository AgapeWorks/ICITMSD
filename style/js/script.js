...
function kboard_editor_open_media() {
    ...
    if (kbaord_current.board_id) {
        if (jQuery('#kboard_media_wrapper').length) {
            ...
            var background = jQuery('<div id="kboard_media_background"></div>').css({
                opacity: '0.5'
            }).click(function() {
                kboard_media_close();
            });
            ...
            if (!jQuery('input[name="media_group"]').filter(function() {
                    return this.value == kboard_settings.media_group
                }).length) {
...
function kboard_comment_like(button, callback) {
    if (!kboard_ajax_lock) {
        kboard_ajax_lock = true;
        jQuery.post(kboard_settings.alax_url, {
            'action': 'kboard_comment_like',
            'comment_uid': jQuery(button).data('uid'),
            'security': kboard_settings.ajax_security
        }, function(res) {
            kboard_ajax_lock = false;
            if (typeof callback === 'function') {
